...
const SELECT_COLUMNS = `
  ...
  tp.plate_number AS "plateNumber", tp.hubs,
  COALESCE(t.status, tp.status) AS "status",
...
async function list({
  onlyActive, search, status, limit, offset,
} = {}) {
  const values = [];
  const whereClauses = [];
  if (onlyActive) {
    whereClauses.push('tp.cancelled_at IS NULL');
    // Exclude this row if it has already been converted into a trip itself...
    whereClauses.push('t.trip_id IS NULL');
    // ...AND exclude it if its assigned vehicle is already running/queued on
    // a DIFFERENT trip planning row. Without this, a vehicle that's already
    // ONGOING on trip planning #11 still shows up as selectable via any of
    // its other (not-yet-converted) planning rows, e.g. #14 for the same
    // plate — which is the "ongoing trip still showing in Select Trip" bug.
    whereClauses.push(`
      NOT EXISTS (
        SELECT 1
        FROM trips t2
        JOIN trip_planning tp2 ON tp2.trip_planning_id = t2.trip_planning_id
        WHERE tp.plate_number IS NOT NULL
          AND tp2.plate_number = tp.plate_number
          AND tp2.trip_planning_id <> tp.trip_planning_id
          AND t2.status IN ('CREATED', 'ONGOING')
      )
    `);
  }
  if (search) {
  ...
  if (status) {
    values.push(status);
    // Match against the *effective* status: once a planning row is converted
    // into a real trip, that trip's status (CREATED/ONGOING/...) is the
    // source of truth — tp.status is only used as a fallback pre-conversion.
    whereClauses.push(`COALESCE(t.status, tp.status) = $${values.length}`);
  }
